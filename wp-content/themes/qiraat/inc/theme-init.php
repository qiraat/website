...
if (!class_exists('Qiraat_Init')) {
    ...
    class Qiraat_Init
    {
        ...
        public function theme_setup()
        {
             ...
             */
            load_theme_textdomain('qiraat', get_template_directory() . '/languages');
            ...
            register_nav_menus(array(
                'main-menu' => esc_html__('Primary Menu', 'qiraat'),
                'main-menu-02' => esc_html__('Primary Menu Two', 'qiraat'),
            ));
            ...
            $GLOBALS['content_width'] = apply_filters('qiraat_content_width', 740);

            //add image sizes
            add_image_size('qiraat_classic', 750, 400, true);
            add_image_size('qiraat_grid', 370, 270, true);
            add_image_size('qiraat_medium', 550, 380, true);
        ...
        public function theme_widgets_init()
        {
            register_sidebar(array(
                'name' => esc_html__('Sidebar', 'qiraat'),
                'id' => 'sidebar-1',
                'description' => esc_html__('Add widgets here.', 'qiraat'),
            ...
            register_sidebar(array(
                'name' => esc_html__('Footer Widget Area', 'qiraat'),
                'id' => 'footer-widget',
                'description' => esc_html__('Add widgets here.', 'qiraat'),
        ...
        {
            $theme_version = QIRAAT_DEV ? time() : qiraat()->get_theme_info('version');
            $css_ext = '.css';
            //load google fonts
            $enqueue_google_fonts = self::load_google_fonts();
            if (!empty($enqueue_google_fonts)) {
                wp_enqueue_style('qiraat-google-fonts', esc_url(add_query_arg('family', urlencode(implode('|', $enqueue_google_fonts)), '//fonts.googleapis.com/css')), array(), null);
            }
            $all_css_files = array(
                array(
                    'handle' => 'animate',
                    'src' => QIRAAT_CSS . '/animate.css',
                    'deps' => array(),
                    'ver' => $theme_version,
                    'media' => 'all',
                ),
                ...
                array(
                    'handle' => 'bootstrap',
                    'src' => QIRAAT_CSS . '/bootstrap.min.css',
                    'deps' => array(),
                    'ver' => $theme_version,
                    'media' => 'all',
                ),
                array(
                    'handle' => 'font-awesome',
                    'src' => QIRAAT_CSS . '/font-awesome.min.css',
                    'deps' => array(),
                    'ver' => '5.12.0',
                    'media' => 'all',
                ),
                array(
                    ...
                    'src' => QIRAAT_CSS . '/magnific-popup.css',
                    'deps' => array(),
                    'ver' => $theme_version,
                    'media' => 'all',
                ),
                array(
                    ...
                    'src' => QIRAAT_CSS . '/main-style' . $css_ext,
                    'deps' => array(),
                    'ver' => $theme_version,
                    'media' => 'all',
                ),
                array(
                    ...
                    'src' => QIRAAT_CSS . '/responsive' . $css_ext,
                    'deps' => array(),
                    'ver' => $theme_version,
                    'media' => 'all',
                ),
            );
            $all_css_files = apply_filters('qiraat_theme_enqueue_style', $all_css_files);
            ...
            wp_enqueue_style('qiraat-style', get_stylesheet_uri());

            if (qiraat()->is_qiraat_core_active()) {
                if (file_exists(QIRAAT_DYNAMIC_STYLESHEETS . '/theme-inline-css-style.php')) {
                    require_once QIRAAT_DYNAMIC_STYLESHEETS . '/theme-inline-css-style.php';
                    require_once QIRAAT_DYNAMIC_STYLESHEETS . '/theme-option-css-style.php';
                    wp_add_inline_style('qiraat-style', qiraat()->minify_css_lines($GLOBALS['qiraat_inline_css']));
                    wp_add_inline_style('qiraat-style', qiraat()->minify_css_lines($GLOBALS['theme_customize_css']));
                }
        ...
        {
            $theme_version = qiraat()->get_theme_info('version');
            $js_ext = QIRAAT_DEV ? '.js' : '.min.js';
            $all_js_files = array(
                array(
                    'handle' => 'bootstrap',
                    'src' => QIRAAT_JS . '/bootstrap.min.js',
                    'deps' => array('jquery'),
                    'ver' => $theme_version,
                    'in_footer' => true,
                ),
                ...
                array(
                    'handle' => 'magnific-popup',
                    'src' => QIRAAT_JS . '/jquery.magnific-popup.js',
                    'deps' => array('jquery'),
                    'ver' => $theme_version,
                    'in_footer' => true,
                ),
                array(
                    'handle' => 'qiraat-main-script',
                    'src' => QIRAAT_JS . '/main' . $js_ext,
                    'deps' => array('jquery'),
                    'ver' => $theme_version,
                    'in_footer' => true,
                ),
            );
            $all_js_files = apply_filters('qiraat_theme_enqueue_script', $all_js_files);
        ...
        public function load_theme_dependency_files()
        {
            $includes_files = array(
                array(
                    ...
                    'file-path' => QIRAAT_TGMA
                ),
                array(
                    'file-name' => 'theme-breadcrumb',
                    'file-path' => QIRAAT_INC
                ),
                array(
                ...
                array(
                    ...
                    'file-path' => QIRAAT_THEME_SETTINGS
                ),
                array(
        ...
        public function gutenbarg_script()
        {
            ...
            wp_enqueue_style('flaticon', QIRAAT_CSS . '/flaticon.css', [], $theme_version, 'all');
            wp_enqueue_style('flynext-gutenbarg', QIRAAT_CSS . '/gutenbarg.css', [], $theme_version, 'all');
        }

    }//end class
    if (class_exists('Qiraat_Init')) {
        Qiraat_Init::getInstance();
    }
}
