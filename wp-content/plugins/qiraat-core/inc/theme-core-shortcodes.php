...
if (!class_exists('Qiraat_Core_shortcodes')) {

    class Qiraat_Core_shortcodes
    {
        ...
        {
            //social post share
            add_shortcode('qiraat_post_share', array($this, 'post_share'));
            //social icon
            add_shortcode('qiraat_social_icon_wrap', array(__CLASS__, 'social_icon_wrap'));
            add_shortcode('qiraat_social_icon', array(__CLASS__, 'social_icons'));
            //top menu
            add_shortcode('qiraat_top_menu_wrap', array(__CLASS__, 'top_menu_wrap'));
            add_shortcode('qiraat_top_menu', array(__CLASS__, 'top_menu'));
            //top menu
            add_shortcode('qiraat_top_menu_wrap_02', array(__CLASS__, 'top_menu_wrap_02'));
            add_shortcode('qiraat_top_menu_02', array(__CLASS__, 'top_menu_02'));
            //info_item
            add_shortcode('qiraat_info_item_wrap', array(__CLASS__, 'info_item_wrap'));
            add_shortcode('qiraat_info_link', array(__CLASS__, 'info_link'));
            add_shortcode('qiraat_info_inline_text', array(__CLASS__, 'info_inline_text'));
        ...
        public static function post_share($atts, $content = null)
        {
            ...
            if (is_singular() || is_home()) {
                ...
                $qiraat_url = urlencode_deep(get_permalink());
                //get current page title
                $qiraat_title = str_replace(' ', '%20', get_the_title($post->ID));
                //get post thumbnail for pinterest
                $qiraat_thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'full');
                $qiraat_thumbnail = !empty($qiraat_thumbnail) ? $qiraat_thumbnail[0] : '';

                //all social share link generate
                $facebook_share_link = 'https://www.facebook.com/sharer/sharer.php?u=' . $qiraat_url;
                $twitter_share_link = 'https://twitter.com/intent/tweet?text=' . $qiraat_title . '&amp;url=' . $qiraat_url . '&amp;via=' . get_bloginfo('url');
                $linkedin_share_link = 'https://www.linkedin.com/shareArticle?mini=true&url=' . $qiraat_url . '&amp;title=' . $qiraat_title;
                $pinterest_share_link = 'https://pinterest.com/pin/create/button/?url=' . $qiraat_url . '&amp;media=' . $qiraat_thumbnail . '&amp;description=' . $qiraat_title;
    ...
    if (class_exists('Qiraat_Core_shortcodes')) {
        Qiraat_Core_shortcodes::getInstance();
    }
